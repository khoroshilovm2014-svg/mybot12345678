...
async def start(update: Update, context: CallbackContext):
    ...
    if user_id not in data["users"]:
        ...
        data["users"][user_id] = {
            ...
            "pending_referral": new_referrer if new_referrer else None,
            "coins_pending_approval": False,
            "last_receive": None
        }
        save()
    else:
        if user_id in data.get("failed_deliveries", {}):
            del data["failed_deliveries"][user_id]
            save()
...
📝 Текст FAQ: {len(data['settings']['faq_text'])} символов"""
            ...
            if not context.user_data.get("broadcast_msg_id") and not context.user_data.get("broadcast_msg_text"):
